...
def train_gensim(X, size=100, min_count=5):
    """
    Train a FastText model from scratch with Gensim
    :param X: A list of tokenized texts (i.e. list of lists of tokens)
    :return: A trained FastText model
    """
    print("Training FastText...")
    model = gensim.models.FastText(X, size=size, workers=8, min_count=min_count)
    model.save('data/embedding/fasttext/gensim_size%s_min%s' % (size, min_count))
    return model


def load_gensim(model_path='data/embedding/fasttext/gensim_size100_min5'):
...
if __name__ == '__main__':
    ...
    # w2v = load_gensim('data/embedding/fasttext/gensim_size50_min5')
    print("Custom:")
    ...
    w2v = load_gensim('data/embedding/fasttext/gensim_size300_min5')
    print("Google:")
    print("Vocab. size:", len(w2v.wv.vocab))
    print('repo', [x[0] for x in w2v.most_similar('repo')])
    print('fork', [x[0] for x in w2v.most_similar('fork')])
