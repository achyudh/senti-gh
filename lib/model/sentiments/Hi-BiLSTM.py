...
from tensorflow.python.keras.layers import Dropout, Bidirectional, TimeDistributed
...
def train(train_x, train_y, evaluate_x, evaluate_y, embedding_map, embedding_dim, max_sequence_len, max_sequences, num_classes):
    with tf.device('/gpu:0'):
        ...
        l_lstm_1 = Bidirectional(LSTM(100, dropout=0.2, recurrent_dropout=0.2))(embedded_sequences_1)
        encoder_1 = Model(sequence_input_1, l_lstm_1)
        ...
        l_lstm_2 = Bidirectional(LSTM(20))(encoder_2)
        ...
        checkpoint_callback = ModelCheckpoint(filepath="data/models/hi_bilstm/%s.hdf5" % dataset_name, monitor='val_acc', verbose=1, save_best_only=True)
        model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
        model.summary()
        model.fit(train_x, train_y, validation_data=(evaluate_x, evaluate_y), epochs=20, batch_size=64,
                  callbacks=[early_stopping_callback, checkpoint_callback])
    model.load_weights("data/models/hi_bilstm/%s.hdf5" % dataset_name)
    return model
