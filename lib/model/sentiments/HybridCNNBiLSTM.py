...
def initialize(embedding_map, tokenizer, config, freeze_encoder=False):
    ...
    l_conv1 = Conv1D(100, 7, activation='relu', padding='valid')(embedded_sequences_1)
    l_pool1 = GlobalMaxPool1D()(l_conv1)
    l_conv2 = Conv1D(100, 5, activation='relu', padding='valid')(embedded_sequences_1)
    ...
    sequence_output_1 = TimeDistributed(encoder_1)(sequence_input_2)
    l_lstm_2 = Bidirectional(LSTM(config.hidden_dim, dropout=0.2, recurrent_dropout=0.2))(sequence_output_1)
    encoder_2 = Model(sequence_input_2, l_lstm_2)

    sequence_output_2 = encoder_2(sequence_input_2)
    # l_dense2 = Dense(128, activation='relu')(sequence_output)
    # l_dropout2 = Dropout(config.dropout_rate)(l_dense2)
    l_dense3 = Dense(config.num_classes, activation='softmax')(sequence_output_2)
    model = Model(sequence_input_2, l_dense3)

    if freeze_encoder:
        for layer in encoder_1.layers:
    ...
    encoder_2.summary()
    model.summary()
    return model, encoder_1, encoder_2
...
def train(model, train_x, train_y, evaluate_x, evaluate_y, config):
    ...
    checkpoint_callback = ModelCheckpoint(filepath=os.path.join(config.checkpoint_path, "HybridCNNBiLSTM-%s.hdf5" % config.dataset),
                                          monitor='val_acc',
                                          verbose=1,
    ...
    model.load_weights(os.path.join(config.checkpoint_path, "HybridCNNBiLSTM-%s.hdf5" % config.dataset))
    return model
...
def cross_val(data_x, data_y, embedding_map, tokenizer, config, n_splits=5):
    ...
    for train_index, test_index in skf.split(data_x, data_y.argmax(axis=1)):
        ...
        model, encoder_1, encoder_2 = initialize(embedding_map, tokenizer, config)
...
def transfer_learn(data_x, data_y, source_data_x, source_data_y,embedding_map, tokenizer, config, n_splits=5):
    source_model, source_encoder_1, source_encoder_2 = initialize(embedding_map, tokenizer, config)
    train_x, test_x, train_y, test_y = train_test_split(source_data_x, source_data_y,
                                                        test_size=0.2,
                                                        random_state=157,
                                                        stratify=source_data_y)
    train(source_model, train_x, train_y, test_x, test_y, config)
    ...
    for train_index, test_index in skf.split(data_x, data_y.argmax(axis=1)):
        ...
        model, encoder_1, encoder_2 = initialize(embedding_map, tokenizer, config, freeze_encoder=True)
        encoder_1.set_weights(source_encoder_1.get_weights())
        # encoder_2.set_weights(source_encoder_2.get_weights())
